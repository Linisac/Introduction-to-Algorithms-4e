...
\begin{enumerate}[\thesection-1]
%
\item First note that the set $\setm{\preimage{f}(b)}{b \in f(A)}$ forms a partition of $A$, where $\preimage{f}(b) = \setm{a \in A}{f(a) = b}$ denotes the \emph{preimage} of $b$ under $f$. Thus,
\[
\begin{array}{llll}
\card{A} & =    & \ds{\sum\limits_{b \in f(A)} \card{\preimage{f}(b)}} & \mathtext{(since the sets \mathmode{\preimage{f}(b)} are mutually disjoint and their union is \mathmode{A})} \cr
...
%%
\item Since $f(A) \subseteq B$, we have $\card{f(A)} \leq \card{B}$. If $f$ is injective, i.e., if $\preimage{f}(b)$ is a singleton set for every $b \in f(A)$, then we have $\card{A} = \card{f(A)}$ and hence $\card{A} \leq \card{B}$.
%%
\item If $f$ is surjective, then we have $\card{f(A)} = \card{B}$ and hence $\card{A} \geq \card{B}$.
%%
\end{enumerate}
%
\setcounter{enumi}{2}
