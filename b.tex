...
%%
\item If $f$ is surjective, then $\setm{\preimage{f}(b)}{b \in B}$ forms a partition of $A$, where $\preimage{f}(b) = \setm{a \in A}{f(a) = b}$ denotes the \emph{preimage} of $b$ under $f$. Thus,
\[
\begin{array}{llll}
\card{A} & =    & \ds{\sum\limits_{b \in B} \card{\preimage{f}(b)}} & \mathtext{(since the sets \mathmode{\preimage{f}(b)} are pairwise disjoint and their union is \mathmode{A})} \cr
         & \geq & \ds{\sum\limits_{b \in B} 1} & \mathtext{(since the sets \mathmode{\preimage{f}(b)} are nonempty)} \cr
         & =    & \card{B}. & \cr
\end{array}
\]
%%
