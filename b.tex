...
\setcounter{enumi}{4}
%
\item Use induction on the size of $S$ or use a counting method based on the rule of product (see section C.1).
%
\end{enumerate}
%End of Section B.1-----------------------------------------------------------------------------------------------------
...
\begin{enumerate}[\thesection-1]
%
\item Let us observe the following:
\begin{enumerate}[(1)]
%%
\item The set $\setm{\preimage{f}(b)}{b \in f(A)}$ forms a partition of $A$, where $\preimage{f}(b) = \setm{a \in A}{f(a) = b}$ denotes the \emph{preimage} of $b$ under $f$. Thus,
\[
\begin{array}{llll}
\card{A} & =    & \ds{\sum\limits_{b \in f(A)} \card{\preimage{f}(b)}} & \mathtext{(since the sets \mathmode{\preimage{f}(b)} are pairwise disjoint and their union is \mathmode{A})} \cr
         & \geq & \ds{\sum\limits_{b \in f(A)} 1} & \mathtext{(since the sets \mathmode{\preimage{f}(b)} are nonempty)} \cr
         & =    & \card{f(A)}, & \cr
\end{array}
\]
for which the equality holds if and only if $f$ is injective, i.e., if and only if $\preimage{f}(b)$ is a singleton set for every $b \in f(A)$.
%%
\item Since $f(A) \subseteq B$, we have $\card{f(A)} \leq \card{B}$, for which the equality holds if and only if $f$ is surjective, i.e., if and only if $f(A) = B$.
%%
\end{enumerate}
Now the two parts of this exercise easily follow.
\begin{enumerate}[\it a.]
%%
\item If $f$ is injective, equivalently, if $\card{A} = \card{f(A)}$, then $\card{A} \leq \card{B}$.
%%
\item If $f$ is surjective, equivalently, if $\card{f(A)} = \card{B}$, then $\card{A} \geq \card{B}$.
%%
\end{enumerate}
%
\setcounter{enumi}{2}
...
%
\item By strong induction on the number of vertices $\card{V}$. In the inductive step, consider an arbitrary vertex $v$ from $G$ and the graph induced by $V - \sete{v}$, which consists of $n$ connected components for some $n \geq 1$; each of these connected components is connected to $v$ in $G$ via at least one edge.
%
\item (INCOMPLETE) Verification part skipped. Reflexivity and transitivity hold in the case of directed graphs.
%
\item INCOMPLETE
%
